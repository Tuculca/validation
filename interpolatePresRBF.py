...
def plot(fileInput):
	...
	obs_15 = obs_15.drop(obs_15[obs_15.VALUE < -1].index)
	...
	norm = colors.BoundaryNorm(numpy.arange(750, 1100, 10), cmap.N)
	
	heights = wrf.getvar(file_nc, 'HGT')
	print("Max interpolated value: " + str(grid_values.max()))
	print("Min interpolated value: " + str(grid_values.min()))
